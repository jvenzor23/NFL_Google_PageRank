...
# creating unique game codes
pbp2018ordered = pbp2018 %>%
  arrange(game_date)
game_codes = unique(pbp2018ordered$game_id)
codes = seq(length(game_codes))
game_num = data.frame("game_id" = game_codes,"game_number" = codes)

# getting rid of first 4 games for every team
pbp2018 = pbp2018 %>%
  inner_join(game_num, by = "game_id") %>%
  filter(game_number > 72)
...
# CHECKING THAT A COLUMN IS NOT EMPTY
min(colSums(output))
...
write.csv(output, "Pij_4R_2018.csv")

# Adjusting for outlier scores
output[output >20] = 20
write.csv(output, "Pij_4R_outlier_removed_2018.csv")